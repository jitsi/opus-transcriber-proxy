...
- **Long-turn cap (`XAI_MAX_TURN_MS`, default 15000; 0 disables):** in the default one-final-per-turn mode (and always when diarized), xAI commits each segment of a turn with `is_final=true` but only the end-of-turn `speech_final` (carrying the whole turn) produces a final, so a speaker who never pauses long enough for xAI to call end-of-speech gets no final at all — the common case since 2026-09-19, when xAI stopped sending `speech_final` at the short pauses it used to. `XAIBackend` holds a turn's committed segments; once the turn is older than the cap they are emitted as a final (by a per-turn timer, or by the commit that finds the turn past the cap), and each further segment is emitted as it commits. These early finals are reported as `midUtterance`, so the idle force-commit still finalizes the rest if the speaker stops. The later `speech_final` (or a mid-turn `transcript.done`) emits only the rest, found by aligning the last emitted words in the whole-turn text — in the `words` entries when diarized, otherwise the text's words by UAX #29 word boundaries (`Intl.Segmenter`, so languages written without spaces align too) — because xAI may re-punctuate or re-tokenise it, so it is not always a literal prefix. The anchor is accepted only within `anchorSlack` of where the emitted word count puts it (three words, or a quarter of the count for a long turn): a three-word phrase can recur earlier in a turn ("I think that"), and taking a recurrence for the emitted tail would cut the rest in the wrong place with no warning. The text rest is cut from the original string right after the last emitted word, minus what closes it, so punctuation that opens the rest ("¿", a quote) is kept. If those words cannot be found, a `speech_final` that starts with exactly the turn's first few words is the whole turn and the emitted word count is dropped from it; any other carries only the turn's tail and is emitted whole (a repeat is preferred to a loss). Both fallbacks warn, with counts only. If xAI answers the idle silence with no `speech_final`, the turn's held segments are flushed and its clock stopped a few seconds later, so later segments are not all emitted at once as past the cap; what was emitted is kept (`carried`) so a late `speech_final`/`transcript.done`, or a turn xAI keeps open after the speaker resumes, is reconciled rather than repeated, while a fresh turn (which lacks the carried words) is emitted whole; a record that is *only* carried words needs its full three-word anchor near where the count puts it (or an exact repeat of a short turn), so a turn of fewer than three words that xAI folds into the next `speech_final` is repeated rather than risked. Once a new turn's early finals are recorded on top of a carried record (`carriedCount` says how much was carried), its anchor is the new turn's and is accepted at either the whole count (xAI folded the ended turn in) or the new turn's count alone (a fresh turn), so a fresh turn's early finals are not repeated. The idle silence is skipped when a turn already ended after the last audio and no turn has opened since. Held segments are joined without a space between two pieces of a language written without spaces (zh/ja/th; judged on whole code points, so Han outside the BMP counts). On the diarized path `words` entries are split on the same word boundaries as text, so a held segment with no speaker labels aligns against a labelled `speech_final`; it also keeps the speaker the turn was last emitted under. `XAI_MAX_TURN_MS=0` restores the pre-cap behaviour: no turn clock, nothing is held, no idle turn end, no silence skip (in granular mode too), and a non-empty `transcript.done` is always emitted; only the `midUtterance` flag on granular commits remains, a granular fix of its own whose rollback is `XAI_GRANULAR_FINALS`. A speaker whose `language` arrives only on the `speech_final` still gets it on the next turn's early finals (`lastLanguage` is taken from a turn end too). What was emitted is kept as a count plus the first and last few words, so a turn with no `speech_final` does not grow memory. When diarized, an `is_final` segment's trailing words often carry no `speaker` (the `speech_final` labels all of them); an unlabelled word takes the speaker of the word before it, so a segment is not cut into two finals at the first unlabelled word. The cap timer logs at info when it fires with nothing held. A committed segment that arrives without `words` keeps the speaker of the words before it. A held segment is also flushed, as the end of the utterance, when the stream errors (API error or live-socket error) or xAI closes the socket, and before a non-empty `speech_final` that carries neither its first nor its last words (xAI reset mid-turn, so the `speech_final` holds only what followed; a warn with counts), and an empty `speech_final` or `transcript.done` ends the turn (a non-empty `transcript.done` right after a `speech_final`, with no audio in between, is reconciled against that turn: the same text emits nothing, a tail emits the tail, and a turn never seen in a partial is emitted whole). The idle turn end is not armed when the silence could not be sent. Committed segments are held as they come; one that starts with the previous one's words is a speaker repeating themselves, not an accumulating segment, and is kept. An owner-driven teardown drops what is held, exactly like the uncommitted interim text. A turn that ends inside the cap is emitted exactly as before. Global-only (no URL param)
