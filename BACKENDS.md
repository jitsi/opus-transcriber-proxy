...
- `forceCommit()` sends `{"type": "audio.done"}` to flush and finalize the last pending utterance when the audio stream goes idle. xAI closes the connection after `audio.done` (observed code 1006); it is recreated on the next media event. #94 had made this a no-op; even with #95's `endpointing`-based finalization the trailing utterance before a pause/mute was left unfinalized once audio stopped, so the explicit idle commit was restored. xAI's own `"ASR stream timed out"` on a long idle is handled by the recoverable-reconnect path
