...
- `forceCommit()` is a **no-op** for xAI (no multiplexing — one WS per stream). `{"type": "audio.done"}` signals end-of-stream: xAI finalizes then closes the WS (code 1006). Sending it on every idle commit (`FORCE_COMMIT_TIMEOUT`, default 2s) tore the stream down on every silence and dropped the post-unmute speech burst before xAI transcribed it. xAI finalizes utterances on silence via `endpointing` (see below), so no manual commit is needed; keeping the stream open across silences avoids the churn (JIT-15901). When the stream is genuinely idle long enough, xAI's own `"ASR stream timed out"` eventually fires and is handled by the recoverable-reconnect path above
