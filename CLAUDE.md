...
- **Close flush**: `flushTranslationUsage()` drains the final delta + any sub-threshold buffer. Node calls it on `SIGTERM`; the Worker calls `void flushTranslationUsage()` in `handleTranslate`'s `proxy.on('closed', ...)` handler (fire-and-forget — no `ExecutionContext` there). Without it the last interval's usage would be lost when the isolate is torn down.
