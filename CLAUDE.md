...
- **Handshake failures: diagnostics + bounded retry.** xAI rejects the WS upgrade with an HTTP response when its STT backend is unavailable (observed at scale on 2026-09-10: fleet-wide `503`s). `XAIBackend` attaches a `ws` **`unexpected-response`** listener, so it owns the abort (that listener suppresses `ws`' generic `"Unexpected server response: <code>"` error) and logs everything xAI support asks for on a failed call: `status`, `requestId` (first of `x-request-id`/`x-requestid`/`request-id`/`x-amzn-requestid`/`cf-ray`), `retryAfter`, the full response `headers`, the (capped at 2KB / 1s) response `body`, and the request `url` incl. query params. The **successful** handshake's request id is captured from the `upgrade` event into `this.requestId` and appended to every later failure log for that stream (`xAI WebSocket error …`, `xAI API error …`, close), so a mid-stream failure is traceable too. Retry: `connect()` makes up to `XAI_CONNECT_ATTEMPTS` (4) attempts with exponential backoff (`XAI_CONNECT_BACKOFF_MS`, 250ms base, doubling, ±25% jitter, capped 4s — ~1.75s worst case). A `Retry-After` on the rejection (delay-seconds **or** HTTP-date, per RFC 9110) **wins over** the computed backoff, but only up to 4s: a longer ask is treated as "don't retry" and the connect fails, because audio is buffering behind `connect()` and stalling seconds in there is worse than failing and letting the next media event open a fresh connection for transient upgrade statuses (408/425/429/500/502/504 **and 503** — per xAI, a 503 means "temporarily unavailable, retry with backoff", explicitly not a 429 — **plus the Cloudflare 52x origin-failure codes** 521/522/523/524/525/526/530, since `api.x.ai` is behind Cloudflare and its edge answered 521+525 on 2026-09-04, 522 on 09-08 and 521 on 09-09; those never reach xAI's API and are equally transient) and for pre-open transport errors; 401/403/400/404 and a malformed `XAI_STT_URL` fail fast. A failed attempt's socket is **abandoned** (`abandoned` flag): its late `error`/`close` events log at debug and must never reach `onError`/`onClosed`, or the retried connection would be torn down under us. After the last attempt the historical behaviour is unchanged — `onError('websocket_error', …)` (errorType deliberately kept stable so `otp_backend_errors_total{type="websocket_error"}` and the alerts on it keep their meaning; the status + request id ride along in the *message*) then `close()`. A connect failure stays **fatal** (not `recoverable`): reporting it as recoverable would race `initializeBackend`'s own catch, which tears the connection down. No model or region is pinned on the request (xAI's STT default applies)
