...
- **Granular finalization (roll-own, `XAI_GRANULAR_FINALS`, default OFF):** xAI commits a final only on its end-of-turn `speech_final`, which re-emits the WHOLE turn at once — so a long speaker's turn lands in the stored transcript AFTER other participants' short acks (the GT-meeting ordering bug). When enabled, `XAIBackend` instead runs `XAIGranularSegmenter` (`src/backends/XAIGranularSegmenter.ts`): it reconstructs xAI's growing hypothesis from the interim stream (chunk_finals are segment-wise and interims reset after each — `mergeBase()` handles both that and a hypothetical cumulative provider), commits a **stable prefix** once it's been unchanged for `XAI_GRANULAR_STABILITY_MS` (default 1000) holding back `XAI_GRANULAR_GUARD_WORDS` (default 3) volatile words, and batches frozen words into `XAI_GRANULAR_MIN_WORDS` (default 5)-word segments emitted as finals (the in-progress remainder is emitted as an interim, Deepgram-style). The end-of-turn `speech_final` is **reconciled** — only the trailing words not already committed are flushed from its authoritative text, so the whole-turn re-emit is never reprinted; reconciliation only appends, so the sole correctness cost is a prefix word xAI revised after we froze it (measured **0 word-edits** across the whole tuned grid on 12 live captures — the "slower ⇒ fewer edits" framing is moot, the floor is already 0; slower only buys latency-margin). A non-empty `transcript.done` is routed through the segmenter only when a turn is still active (`hasActiveTurn()`) to avoid duplicating a turn already ended by `speech_final`. Granular finals are scoped to the **non-diarized** path (diarization needs per-speaker hypotheses → falls back to one-final-per-turn). The recoverable stream-timeout reconnect is unchanged. Two things the long-turn cap work changed apply here too: granular commits made before the end of a turn are reported as `midUtterance`, so the owner keeps its idle force-commit armed after them, and, with the long-turn cap on, the DTX silence-injection `forceCommit()` is skipped when the granular turn already ended after the last audio sent and no turn has opened since (the segmenter's `hasActiveTurn()`, since the granular path never starts the cap's turn clock). Flag + the stability/guard knobs are overridable **per-connection** via `xai_granular_finals`/`xai_granular_stability_ms`/`xai_granular_guard_words` URL params (resolved `backendConfig.xaiGranularX ?? config.xai.granularX`); `XAI_GRANULAR_MIN_WORDS` is **global-only** (a batching detail, not per-connection). Tuned live in `unreal-agents/experiments/xai-vs-deepgram-finalization` (see `TUNING.md`)
- **Long-turn cap (`XAI_MAX_TURN_MS`, default 15000; 0 disables):** in the default one-final-per-turn mode (granular off, and always when diarized), xAI commits each segment of a turn with `is_final=true` (the interim text resets after each) and ends the turn with a `speech_final=true` whose text is the whole turn — normally the only final. Since **2026-09-19** a continuous speaker may never get one, and since jicofo connects with `sendBack=true` but not `sendBackInterim` the bridge then receives nothing (the stage monitor's identical replay dropped from 45 interims to 11-13 per run between 2026-09-18 22:01Z and 2026-09-19 00:01Z). So `XAIBackend` holds the turn's committed segments and, once the turn is older than the cap, emits them as a final (per-turn timer, or the commit that finds the turn past the cap) and each further segment as it commits; the later `speech_final` emits only the rest. The design narrative, xAI's observed event shapes and every edge case live in **BACKENDS.md ("Long-turn cap")**; the invariants to keep are: (1) **early finals are `midUtterance`** (second argument of `onCompleteTranscription`; granular commits before the end of a turn too), so `OutgoingConnection` keeps its idle force-commit armed after them; (2) **the rest of a turn is found by aligning words, never by a literal prefix or a raw count** (`alignTurnRest`: the last emitted words, located within `anchorSlack` of where the count puts them, in UAX #29 word units on both the text and the `words`-entry path; the text rest is cut from the original string right after the last emitted word, dropping what closes it and keeping what opens the rest); its fallbacks prefer a repeat to a loss (a `speech_final` is the whole turn only when it starts with **exactly** the turn's first words; anything else is emitted whole) and **warn with word counts only** — transcript text is never logged above debug; (3) **what was emitted is a fixed-size record** (`EmittedTurn`: count, first/last few normalized words, last speaker, and the boundaries (`carriedAt`) at which it was `carried` from a turn that already ended — on idle without a `speech_final`, or as `lastTurn` — so a late `speech_final`/`transcript.done` and a turn xAI folds into the next one are reconciled, while a fresh turn's early finals align against their own count); (4) **held segments are never silently dropped**: they are flushed before `onError`, on a remote close, on an empty `speech_final`/`transcript.done`, on the idle turn end (`forceCommit()` arms it; silence + `XAI_IDLE_TURN_END_GRACE_MS`, default 3s, with no `speech_final`), and before a `speech_final` that does not carry them (xAI reset mid-turn) — except a held segment shorter than an anchor, which cannot be told from a re-rendering and is discarded with a debug line; an owner-driven teardown drops them like the uncommitted interim text it always dropped; (5) **`forceCommit()` skips its silence only when a turn ended after the last audio sent and no turn has opened since** (the segmenter's `hasActiveTurn()` on the granular path); (6) **`XAI_MAX_TURN_MS=0` restores the pre-cap behaviour**: no turn clock, nothing held, no idle turn end, no silence skip (granular included), a non-empty `transcript.done` always emitted — only the `midUtterance` flag on granular commits remains, since that fixes a granular bug of its own and `XAI_GRANULAR_FINALS` is its rollback. `lastLanguage` is also taken from a `speech_final`/`transcript.done`, so an early final of the next turn carries the right language. Segments are joined with `joinText` (no space between two pieces of a language written without spaces; whole code points, so Han outside the BMP counts). Global-only (no URL param)
- The CF Worker forwards `XAI_API_KEY` (as `''` when unset → provider disabled) plus any set `XAI_STT_URL`/`XAI_LANGUAGE`/`XAI_DIARIZE`/`XAI_INCLUDE_LANGUAGE`/`XAI_ENDPOINTING`/`XAI_SMART_TURN`/`XAI_SMART_TURN_TIMEOUT`/`XAI_GRANULAR_FINALS`/`XAI_GRANULAR_STABILITY_MS`/`XAI_GRANULAR_GUARD_WORDS`/`XAI_GRANULAR_MIN_WORDS`/`XAI_CONNECT_ATTEMPTS`/`XAI_CONNECT_BACKOFF_MS`/`XAI_MAX_TURN_MS`/`XAI_IDLE_TURN_END_GRACE_MS` to the container via `buildContainerEnvVars`
