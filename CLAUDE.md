...
- `close()` is idempotent (guarded by checking/setting `status = 'closed'` before touching the WebSocket), mirroring `OutgoingConnection`/`TranslatorConnection`'s `doClose()` pattern. Found via the integration test harness: on a bad API key, undici's `WebSocket.close()` can synchronously re-dispatch `'error'`/`'close'` on a connection that never opened, and since those listeners call `close()` themselves, an unguarded `close()` recursed without bound and stack-overflowed the whole process (crashing every other session too, not just the one with bad credentials)
