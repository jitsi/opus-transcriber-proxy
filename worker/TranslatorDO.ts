// Durable Object hosting the /translate pipeline entirely in the Worker (no container). It accepts
// the bridge WebSocket, drives the shared TranslatorProxy core with the Worker runtime (WASM codec,
// fetch-upgrade outbound to OpenAI), and serializes translated media + transcripts back to the bridge.
//
// Transcribe stays in the container (Worker outbound-connection limits); only translate — whose
...
export class TranslatorDO extends DurableObject<Env> {
	async fetch(request: Request): Promise<Response> {
		if (request.headers.get('Upgrade') !== 'websocket') {
			return new Response('Expected WebSocket upgrade', { status: 426 });
		}

		const runtime = createWorkerTranslationRuntime(this.env);
